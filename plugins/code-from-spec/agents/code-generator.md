...
```text
Spec 파일: {spec_file_path}          # 단일 Spec 파일 경로
  또는
Spec 디렉토리: {spec_dir}            # Spec 디렉토리 경로
브랜치: {branch_name}                # 작업 브랜치명 (work-scheduler가 생성한 경우)
  또는
출력 디렉토리: {target_dir}          # 브랜치 미지정 시 출력 경로
CLAUDE.md 경로: {claude_md_path}
```

> **isolation: "worktree"로 디스패칭된 경우**: SDK가 격리된 worktree를 자동 생성합니다.
> 브랜치가 지정되면 해당 브랜치로 체크아웃합니다: `git checkout {branch_name}`
> 이후 모든 파일 Read/Write/Bash 작업은 현재 작업 디렉토리 기준 절대경로로 수행합니다.
...
```bash
# 브랜치가 지정된 경우 체크아웃 (isolation: "worktree" 환경에서)
git checkout {branch_name}

# 현재 작업 디렉토리를 WORK_DIR로 사용 (isolation이 격리 보장)
WORK_DIR=$(pwd)
# 브랜치 미지정 + isolation 미사용 시: target_dir 생성
# WORK_DIR={target_dir}
...
`isolation: "worktree"`로 디스패칭된 경우, SDK가 격리된 작업 환경을 제공합니다.

1. **브랜치가 지정된 경우 해당 브랜치로 체크아웃합니다**: `git checkout {branch_name}`
2. 현재 작업 디렉토리(`pwd`)를 기준으로 모든 파일 작업을 수행합니다.
   - Teammate에게도 `WORK_DIR=$(pwd)` 절대경로를 전달합니다.
3. 구현 완료 후 Spec 파일의 구현 추적 섹션을 업데이트합니다. 구현 추적 업데이트는 **빌드+테스트 통과 후**에 수행한다. 빌드 실패 상태에서 추적을 업데이트하지 않는다.
   - `status`: `in-progress` → `completed`
   - `commit`: 최종 커밋 SHA
4. Jira 티켓 상태를 `In Progress` → `In Review`로 전이합니다.
