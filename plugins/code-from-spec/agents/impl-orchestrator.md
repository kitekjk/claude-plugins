...
| 통합, integrate, integration, 통합 테스트, 통합 브랜치 | `integrate` | (직접 실행 — Git 머지 + 테스트) |
| 전체, full, 구현+검증 | `full` | `work-scheduler` → `code-generator` → `spec-verifier` → `spec-feedback` → `integrate` |
...
>
> **강제 메커니즘**: 각 단계 완료 시 다음 정보를 기록한다: 단계명, 실행 여부(실행/스킵), 스킵 사유. 모든 단계가 '실행'으로 기록되어야 다음 워크플로우로 넘어간다. '스킵'이 1건이라도 있으면 워크플로우 실패로 판정한다.
...
>
> **코드 변경의 정의**: 소스 코드 파일(.java, .kt, .ts, .tsx, .py 등), 빌드 설정 파일(build.gradle, pom.xml, package.json 등), 설정 파일(application.yml, .env 등)의 내용이 1줄 이상 추가/수정/삭제된 경우. 주석만 변경된 경우도 코드 변경에 포함한다. 단, .md 파일, .gitignore 등 문서/메타파일만 변경된 경우는 제외한다.
...
> **절대 규칙 3 — 에이전트 격리**: 에이전트를 디스패칭할 때 반드시 Agent 도구의 `isolation: "worktree"` 파라미터를 사용한다.
> `isolation: "worktree"`는 프로세스 수준에서 작업 디렉토리를 격리하여, 에이전트가 원본 폴더를 수정하는 것을 방지한다.
> 프롬프트에 경로를 전달하는 것만으로는 격리가 보장되지 않는다 — 에이전트가 지시를 무시하고 원본 폴더에서 작업할 수 있다.
> 병렬 실행 시 필수이며, 순차 실행에서도 동일하게 적용한다.
...
1. Spec 디렉토리 존재 여부
2. **Spec 상태 사전 확인** — 모든 Spec이 이미 `completed`/`review-approved`/`verified`이면 사용자에게 알리고 종료. 상태 필터는 contract.json의 scheduling.statusFilter를 사용한다. 'completed'/'review-approved'/'verified' 상태가 아닌 Spec이 1개라도 있으면 실행 대상이 있는 것이다.
...
- 2회 재시도 후에도 누락 시:
  - impl-orchestrator에 'TC-ID 커버리지 게이트 실패' 보고
  - **파이프라인 즉시 중단** — 해당 Spec의 ④ 코드 리뷰로 넘어가지 않는다
  - 사용자에게 누락 TC-ID 목록과 함께 알림
  - 사용자가 명시적으로 '계속' 지시한 경우에만 ④로 진행
  - **자동 재개 금지**
...
**Level별 실행 전략**:
- Level 0: 독립 Spec들을 **병렬**로 각각 ①~④ 사이클 실행 → ⑤ 승인 게이트는 **순차적**으로 각 Spec 결과를 보고. 같은 Level의 Spec은 maxParallel(기본값 3) 이내에서 병렬 실행한다. **병렬 디스패칭 시 `isolation: "worktree"` 필수** (절대 규칙 3). 병렬 실행이 기술적으로 불가능한 경우(단일 에이전트 환경)에도 순차 실행하며 스킵하지 않는다.
- Level 1+: 의존 Spec들을 **순차적**으로 각각 ①~⑦ 사이클 실행
- 한 Spec의 사이클이 ④에서 D등급(70점 미만)이면 즉시 사용자에게 보고 (다음 Spec 진행 여부 확인). D등급 보고는 auto/review-gate 모드 모두에서 필수이다. 보고 후 사용자 응답을 대기한다. 사용자가 '계속'을 지시하지 않으면 해당 Level의 나머지 Spec 실행을 중단한다.

**code-generator 디스패칭 형식**:
- Agent 도구 호출 시 `isolation: "worktree"` 파라미터를 반드시 포함한다 (절대 규칙 3).
- 프롬프트에 다음 정보를 전달한다:
```text
Spec 파일: {spec_file_path}
브랜치: {branch_name}   ← work-scheduler가 반환한 해당 Spec의 브랜치명
CLAUDE.md 경로: {claude_md_path}
```

**⑨ 통합 (integrate)** — 전체 Level 완료 후 자동 실행
- integrate 워크플로우 실행 (base branch 확인 → integration branch 생성 → 머지 → 테스트 → push 확인)

**결과 보고** — 통합 완료 후 종합 리포트 출력
...
2. **1-Level lookahead** (선택적)
   - Level N+1 Spec 중 Level N과 `shared_files` 겹침이 **없는** Spec만 선행 구현 (동일 사이클 적용). 'shared_files 겹침'이란: 두 Spec의 dependsOn.shared_files 목록에 동일한 파일명이 1개 이상 존재하는 경우를 말한다. 경로가 다르더라도 파일명이 같으면 겹침으로 판정한다.
   - 선행 구현된 Spec의 상태는 `completed`로 표시하되, 리뷰 대기에 포함하지 않음

3. **리뷰 체크포인트** ⏸
   - Level N의 각 브랜치에서 PR 생성
...
4. **리뷰 응답 처리**
   ...
   - 수정 피드백: 해당 브랜치에서 수정 반영 (isolation: "worktree"로 격리) → **코드 리뷰 루프 + spec-verifier 재실행** → PR 업데이트 → 다시 리뷰 대기
   - 수정 시 하위 Level에 영향 분석:
     - 수정된 파일이 하위 Spec의 `shared_files`에 포함되면 → 무효화 경고를 출력하고, 해당 lookahead Spec의 상태를 `invalidated`로 변경한다. invalidated Spec은 Level N 리뷰 완료 후 재구현한다.
     - 인터페이스/시그니처 변경 시 → 영향받는 하위 Spec 목록 출력

5. **반복** — 모든 Level 완료까지 1~4 반복

6. **통합 (integrate)** — 모든 Level 완료 후 자동 실행
   - integrate 워크플로우 실행 (base branch 확인 → integration branch 생성 → 머지 → 테스트 → push 확인)
...
- [ ] ⑥ spec-feedback Spec 동기화 (승인된 항목만 반영) — ⑤에서 D등급이 나와도 ⑥은 반드시 실행한다. D등급은 Spec 보강이 더 필요하다는 신호이므로 feedback 스킵 사유가 아니다.
- [ ] ⑦ PR 업데이트 (코드 + Spec 커밋)
```
...
② 코드 수정 (code-generator)
   - 리뷰 피드백을 기반으로 해당 브랜치에서 코드 수정 (isolation: "worktree"로 격리)
...
모든 Spec 브랜치 구현 완료 후, 로컬 통합 브랜치를 생성하여 전체 테스트를 실행합니다.
full 워크플로우의 마지막 단계로 자동 실행되며, 별도 명령어로도 실행 가능합니다.

### 사전 확인

1. 구현 완료된 Spec 브랜치 목록 확인 (구현 추적의 `branch` 필드)
2. work-scheduler의 의존성 분석 결과 확인 (topological sort 순서)

### 실행 흐름

```text
① base branch 확인
   • 사용자에게 "통합 대상 브랜치를 지정해주세요 (예: main, develop)" 질문
   • 기본값 없음 — 반드시 사용자에게 확인
   • 사용자가 응답하지 않으면 파이프라인을 대기 상태로 유지한다. 기본값으로 자동 선택하지 않는다.

② integration branch 생성
   • 이름: integration/{baseBranch}/{YYYYMMDD-HHmmss}
   • base branch에서 분기
   git checkout -b integration/{baseBranch}/{YYYYMMDD-HHmmss} {baseBranch}

③ Level 순서대로 머지
   • topological sort 순서 (Level 0 → 1 → N)
   • 같은 Level 내에서는 Spec ID 순서
   • 각 브랜치를 순차적으로 머지:
     git merge {spec_branch} --no-ff
   • 충돌 발생 시 (충돌이란 git merge 명령의 exit code가 0이 아닌 경우를 말한다):
     → 즉시 중단 (git merge --abort)
     → 충돌 브랜치, 충돌 파일 목록 출력
     → "의존성 순서대로 머지하므로 충돌은 의존성 기술 오류입니다" 안내
     → 사용자에게 의존성 수정 후 재실행 요청

④ 전체 빌드 + 테스트 자동 실행
   • 프로젝트 가이드 문서(CLAUDE.md, build 설정 등)에서 테스트 명령 참조
   • 성공 → ⑤로
   • 실패 시:
     → 실패한 테스트 목록 출력
     → 실패 원인과 관련된 Spec/브랜치 식별
       (실패 파일 ↔ Spec의 "수정 대상 파일" 매칭)
     → 관련 Spec과 브랜치 안내
     → integrate 워크플로우를 중단한다. orchestrator는 integration branch를 삭제하지 않고 그대로 유지한다. 사용자가 수정 후 재실행할 수 있도록 한다.

⑤ 결과 리포트
   • integration branch 이름
   • 머지된 브랜치 목록 (Level별)
   • 테스트 결과 요약 (전체 테스트 수, 통과 수)

⑥ push 여부 확인
   • "테스트 통과했습니다. {integration_branch}를 원격에 push할까요?"
   • 사용자 응답을 무기한 대기한다. 자동 push하지 않는다.
   • 승인 → git push -u origin {integration_branch}
   • 거절 → 로컬에만 유지
```
...
```text
Spec 디렉토리: {spec_dir}           (Spec 파일에서 브랜치 정보 추출)
```

full 워크플로우에서 자동 실행 시, 스케줄링 단계에서 이미 수집한 브랜치 정보를 그대로 사용합니다.
