...
**Level별 실행 전략**:
- Level 0: 독립 Spec들을 **병렬**로 각각 ①~④ 사이클 실행 → ⑤ 승인 게이트는 **순차적**으로 각 Spec 결과를 보고. 같은 Level의 Spec은 maxParallel(기본값 3) 이내에서 병렬 실행한다. 병렬 실행이 기술적으로 불가능한 경우(단일 에이전트 환경)에도 순차 실행하며 스킵하지 않는다.
...
모든 Spec worktree 구현 완료 후, 로컬 통합 브랜치를 생성하여 전체 테스트를 실행합니다.
...
④ 전체 빌드 + 테스트 자동 실행
   ...
   • 실패 시:
     → 실패한 테스트 목록 출력
     → 실패 원인과 관련된 Spec/worktree 식별
       (실패 파일 ↔ Spec의 "수정 대상 파일" 매칭)
     → 관련 Spec과 worktree 경로 안내
