...
### 3. PR 리뷰 피드백 (pr-review)

PR 리뷰 과정에서 코드가 수정되어 **Spec과 달라진 부분**을 추출한 항목.

### 4. 코드 리뷰 Spec 갭 (code-review-gap)

코드 리뷰(또는 PR 리뷰)에서 P1/P2 이슈가 발견되었으나 **Spec에 해당 항목이 명시되지 않은 경우**.
이 경우 "Spec에 없으므로 준수"가 아니라 **Spec이 불완전한 것**으로 간주한다.
코드는 이미 수정되었으므로, 수정된 코드에 맞게 Spec을 보강한다.

**code-review Spec 갭의 범위**: impl-orchestrator의 '코드 리뷰 루프'(3소스 병합 기준)에서 발견된 P1/P2 이슈 중 Spec에 명시되지 않은 항목. code-generator 내부의 Teammate 간 코드 리뷰는 이 범위에 포함되지 않는다. code-generator 내부에서 발견된 이슈는 ambiguity-log에 기록한다.

입력 방식 (택 1):
- **PR URL**: GitHub/GitLab PR의 리뷰 코멘트와 변경 내역을 자동 추출
- **리뷰 노트 파일**: 로컬 마크다운 파일로 리뷰 피드백 전달

각 항목에는:
- **PR/리뷰 출처**: PR URL 또는 리뷰 노트 파일 경로
- **변경 사유**: 리뷰어가 요청한 변경 이유
- **코드 변경 내용**: 실제로 수정된 코드 (before/after)
- **Spec 영향**: 이 변경이 Spec의 어떤 부분과 달라지는지
...
| Spec 흐름/조건 미명시 | usecase/service Spec | 응답 필드 누락, 에러 코드 미정의, 흐름 단계 모호 |
| 도메인 모델 모호 | service-definition / model Spec | 엔티티 관계 불명확, 값 객체 범위 모호 |
| 아키텍처 규칙 부족 | architecture-rules | 레이어 간 통신 규칙 미명시 |
| 네이밍 규칙 부족 | naming-guide | 특정 패턴의 네이밍 미정의 |
| 테스트 시나리오 부족 | Spec 테스트 시나리오 섹션 | 경계값 시나리오 누락 |
| 코드 리뷰 Spec 갭 | 해당 Spec 섹션 | 에러 코드 미명시, 응답 형식 미정의, 검증 규칙 누락 |
...
피드백 완료 후 보고:
- 총 피드백 항목 수 (모호점 로그 + 검증 미준수 + 코드 리뷰 Spec 갭)
- 카테고리별 분포
- 승인/반영된 항목 수
- 수정된 Spec 파일 목록
