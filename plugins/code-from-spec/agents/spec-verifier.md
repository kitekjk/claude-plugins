...
**분류 알고리즘**:
1. Spec에 해당 항목이 명시적으로 기술되어 있는가?
   - Yes + 코드가 다르게 구현 → **구현 오류**
   - Yes + 코드가 구현하지 않음 → **구현 오류**
2. Spec에 해당 항목이 기술되어 있지 않은가?
   - No + 코드에 해당 로직이 필요함 → **Spec 갭**
   - No + 코드에 해당 로직이 없음 → **Spec 갭**
3. Spec 기술이 모호하여 여러 해석이 가능한가?
   - 코드가 합리적 해석 중 하나를 따름 → **Spec 갭** (명확화 필요)
   - 코드가 어떤 해석도 따르지 않음 → **구현 오류**

**Invariant**: 판단이 어려운 경우 **Spec 갭**으로 분류한다. 구현 오류로 분류하면 Spec 보강 기회를 놓치기 때문이다.
