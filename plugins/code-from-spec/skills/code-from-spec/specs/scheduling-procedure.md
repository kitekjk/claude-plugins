...
2. 파일 유형별로 분류합니다:
   ...
   - API Spec (`API-*`)
   - Policy Spec (`POLICY-*`)
   - 기초 Spec (`service-definition`, `architecture-rules`, `naming-guide`)
3. 기초 Spec은 스케줄링 대상에서 제외합니다 (항상 최우선 적용).
...
dependsOn:
  - spec_id: UC-ORDER-001
    type: data       # data: 데이터 모델 의존, api: API 호출 의존, event: 이벤트 의존
    reason: "주문 엔티티를 참조하여 결제 처리"
```
...
```text
Level 0: [UC-ORDER-001, UC-PRODUCT-001]     # 의존성 없음 → 병렬
Level 1: [UC-PAYMENT-001, API-ORDER-001]     # Level 0에만 의존 → 병렬
Level 2: [UC-DELIVERY-001]                   # Level 0, 1에 의존 → 순차
```
