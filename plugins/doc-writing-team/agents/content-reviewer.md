...
## HLD 정합성 평가 (LLD에 연관 HLD가 있을 때만)

LLD에 "연관 HLD" 섹션이 있으면 다음 항목을 추가로 평가합니다.
연관 HLD가 없는 소규모 단독 LLD에서는 이 항목을 **적용하지 않으며** 4점을 다른 항목에 균등 배분합니다.

| ID | 체크 항목 | 검증 방법 |
|----|----------|----------|
| L-30 | 연관 HLD 섹션 포함 | HLD 경로, 제목, 대상 컴포넌트가 명시되어 있는지 확인 |
| L-31 | KDD 추적 | HLD의 각 KDD가 LLD 설계 판단 근거에 매핑되는지 확인. 불일치 시 변경 사유 확인 |
| L-32 | NFR 추적 | HLD NFR 상위 목표가 LLD에서 구체 수치로 분해되었는지 확인 |
| L-33 | 용어 일관성 | HLD Glossary 용어가 LLD에서 동일 정의로 사용되는지 확인 |

HLD 정합성이 **50% 미만이면 must-fix**로 분류합니다.
