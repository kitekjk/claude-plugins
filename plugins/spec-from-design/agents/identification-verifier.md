...
- 제외된 클래스가 적절한 사유와 함께 분류되었는지 확인
  ...
  - 제외 사유가 불명확하면 리포트
  - 명확한 제외 사유란 다음 중 하나에 해당하는 것을 말한다: (1) 진입점 분류 테이블의 '비해당' 유형에 정확히 매칭, (2) 다른 UC의 내부 호출 대상으로 명시적 추적 가능, (3) 프레임워크/라이브러리 코드로 프로젝트 외부 코드. 이 3가지에 해당하지 않으면 '불명확'으로 판정하고 리포트한다.
