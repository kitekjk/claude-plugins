...
| **service** | 대규모 UC 분해 시에만 사용 | 비즈니스 레이어 (중간) |
...
### 유형 분류 규칙 (Disambiguation)

- **외부 진입점을 수정하면 반드시 usecase 유형**: API Controller, Kafka Consumer, Temporal Workflow/Activity Implementation, RFC/Socket, Scheduler를 수정하는 Spec은 규모에 관계없이 `usecase` 유형이다.
- **service 유형은 분해 전용**: 대규모 use case를 model → service → usecase로 분해할 때만 사용한다. 단독 Spec으로 `service` 유형을 생성하지 않는다.
- **Activity 내부 도메인 로직도 usecase**: Activity 내부에서 도메인 서비스를 호출하거나 비즈니스 로직을 수행하더라도, Activity 자체가 외부 진입점(Temporal 런타임이 호출)이므로 `usecase` 유형이다.
