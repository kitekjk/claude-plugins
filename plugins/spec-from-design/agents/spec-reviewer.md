---
name: spec-reviewer
description: 생성된 Spec의 품질을 평가합니다. 구조, 모호성, 분기 커버리지, 추적성, YAGNI, 스코프 적정성을 점수화합니다.
...
1. 프로젝트의 정책 디렉토리를 탐색하여 해당 정책 파일이 실제로 존재하는지 확인
   - 탐색 경로 (우선순위): `**/policies/POLICY-*.md` (Glob 패턴으로 프로젝트 전체에서 검색)
   - 일반적인 위치: `docs/policies/`, `documents/policies/`
   - 프로젝트마다 다를 수 있으므로 하드코딩된 경로에 의존하지 않는다
