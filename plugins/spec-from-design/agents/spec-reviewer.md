...
`${CLAUDE_PLUGIN_ROOT}/skills/spec-from-design/checklists/output-quality.md`를 적용합니다.
...
| Q5 | 테스트 도출 가능성 | 15 | Given-When-Then 형식, TC-ID 부여, 레벨 분류, FR당 1+ 시나리오. **모든 testScenarioRequired 유형**(usecase, refactoring, performance, simplification)에 동일 기준 적용. refactoring/simplification은 동작 동등성+하위호환성, performance는 기능 동등성+지표 달성 시나리오 포함 여부 추가 확인 |
...
> **참고**: 추적성 전수 대조는 ⑨ spec-traceability-verifier가 선행 수행합니다. spec-reviewer는 verifier를 통과한 Spec에 대해 Q6 형식 평가를 수행합니다.

verifier가 **warn** 판정으로 전달한 경고 항목이 있으면 Q6에서 추가 감점합니다:

- `출처:` 필드의 정확성 (실제 LLD 섹션과 일치하는지)
- 기본 흐름-FR 대응의 서술 품질
- 정책-KDD 연결의 명확성

추적성 내용 검증(FR 누락 등)은 verifier가 이미 보장하므로, reviewer는 **서술 품질과 형식**에 집중합니다.
...
## 개선 제안 (PASS 시, 만점이 아닌 경우)
{감점된 카테고리별 구체적 개선 방안. 만점(100점)이면 이 섹션 생략.}
  - {Q-ID} {카테고리명} (-{감점}점): {개선하면 점수를 올릴 수 있는 구체적 방법}
  - {Q-ID} {카테고리명} (-{감점}점): {개선 방법}
```
...
- `${CLAUDE_PLUGIN_ROOT}/skills/spec-from-design/contract.json` — 단일 기준, 합격선, 재시도 횟수
- `${CLAUDE_PLUGIN_ROOT}/skills/spec-from-design/checklists/output-quality.md` — 8카테고리 평가 기준, auto-FAIL 조건
- `${CLAUDE_PLUGIN_ROOT}/skills/spec-from-design/checklists/traceability.md` — 추적성 검증 기준
- `${CLAUDE_PLUGIN_ROOT}/skills/spec-from-design/CLAUDE.md` — 역할 분리 규칙 (reviewer는 수정 금지)
