...
**Validation**: TC-ID 생성 전 Spec의 기본 정보에서 도메인과 번호를 확인한다. 비어있거나 형식이 맞지 않으면 orchestrator에 보고하고 Spec 수정을 요청한다. TC-ID를 임의로 생성하지 않는다.
...
- **refactoring**: 최소 기존 기본 흐름의 모든 FR에 대한 동작 동등성 시나리오 + 하위호환성 시나리오 1개 이상
- **performance**: 기능 동등성 시나리오 + 성능 지표 달성 시나리오 1개 이상 (지표와 기대값 명시 필수)
- **simplification**: 제거 대상 동작 불가 확인 시나리오 + 대체 동작 확인 시나리오
