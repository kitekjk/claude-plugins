...
### 식별 단위: 각 구현 클래스 = 하나의 API

REST Controller의 각 엔드포인트가 별도 API인 것처럼, **각 구현 클래스는 하나의 API**입니다.

- 각 ActivityImpl = 별도 API → 별도 use case
- 각 Kafka Consumer = 별도 API → 별도 use case
- 각 Workflow Implementation = 별도 API → 별도 use case

유형(예: "Temporal Activity")으로 묶어서 1개로 만들지 않습니다.
