...
- **외부 진입점을 수정하는 Spec은 반드시 `usecase` 유형**입니다. `service`나 다른 유형으로 분류하지 않습니다.
- Activity 내부에서 도메인 로직(상태 전이, 검증 등)을 수행하더라도, Activity 자체가 외부 진입점(Temporal 런타임이 호출)이므로 `usecase` 유형입니다.
- `service` 유형은 대규모 use case를 분해(model → service → usecase)할 때만 사용합니다. 단독으로 `service` 유형 Spec을 생성하지 않습니다.
