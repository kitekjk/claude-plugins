...
Spec의 모든 Use Case / API 엔드포인트가 구현되었는지 확인합니다.

**검증 항목:**
- Use Case Spec에 정의된 모든 유스케이스가 구현 코드에 존재
- API Spec에 정의된 모든 엔드포인트가 Controller에 존재
- 각 API의 요청/응답 필드가 Spec과 일치
- 에러 응답이 Spec에 정의된 대로 처리됨

**평가:**
- 구현된 Use Case 비율 × 20점
- 구현된 API 엔드포인트 비율 × 20점
